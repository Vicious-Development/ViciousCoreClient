...
public class EntityModelOverrideCFG<T extends ModelBase> {
    ...
    public EntityModelOverrideCFG(Path f, Class<T> modelclass) {
        ...
        for (Field rendererField : rendererFields) {
            ...
            modelPartConfigs.putIfAbsent(name, new EntityPartTransformOverrideCFG(ClientDirectories.directorize(f.toAbsolutePath().toString(), name + ".json")));
        }
    }
    ...
    public EntityModelOverrideCFG(Path f, Class<T> modelclass, int[] arraySizes) {
        ...
        for (Field rendererField : rendererFields) {
            ...
            else modelPartConfigs.putIfAbsent(name, new EntityPartTransformOverrideCFG(ClientDirectories.directorize(f.toAbsolutePath().toString(), name + ".json")));
        }
    }
